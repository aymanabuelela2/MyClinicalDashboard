...
#                 ),
#                 conditionalPanel(
#                     condition = "input.tdap_pbf == 'Pregnant'",
#                     sliderInput(
#                         "tdap_pregWks",
#                         "Weeks: ",
...
#                 ),
#                 selectizeInput(
#                     inputId = "tdap_allergies",
#                     label = "Allergies:",
#                     choices = NULL,
#                     multiple = TRUE
#                 ),
#                 radioButtons("tdap_prior", "Prior treatment for UTI", c("Yes", "No", "other")),
#                 conditionalPanel(
#                     condition = "input.tdap_prior == 'Yes'",
#                     selectizeInput("tdap_priorMed","Medication", choices = NULL),
#                     textInput("tdap_effect", "Effect", placeholder = "Effective/Not effective"),
#                     textInput("tdap_tolerance", "Tolerance", placeholder = "Tolerable/Not tolerable")
#                 ),
#                 conditionalPanel(
#                     condition = "input.tdap_prior == 'other'",
#                     textAreaInput("tdap_priorOther", "Upon asking patient indicated that: ")
#                 )
#             ),
#             
...
#                 ),
#                 conditionalPanel(
#                     condition = "input.tdap_decision == 'Treat'",
#                     textAreaInput(
#                         width = '800px',
#                         "tdap_rationale1",
#                         "Rationale", 
...
#                 ),
#                 conditionalPanel(
#                     condition = "input.tdap_decision == 'Refer'",
#                     textAreaInput(
#                         width = '800px',
#                         "tdap_rationale2",
#                         "Rationale", 
...
#             tabPanel(
#                 "Resources",
#                 includeMarkdown("./tdap/tdap_resources.Rmd")
#             )
#         )
#     ),
# 
...
#             collapsible = TRUE,
#             fluidRow(
#                 column(width = 5, numericInput("tdap_fuSchedule", "Follow up in ", value = 3, min = 0, max = 99)),
#                 column(width = 3, selectInput("tdap_fuUOT", "_", c("minutes", "hours", "days", "weeks", "months"), selected = "days")),
#                 column(
#                     width = 4, 
#                     selectInput(
#                         "tdap_fuMeans",
...
#                 title = "DAP note",
#                 h4("Data:"),
#                 textOutput("tdap_dataHx"),
#                 textOutput("tdap_dataHx2"),
#                 textOutput("tdap_dataHx3"),
#                 textOutput("tdap_dataSx"),
#                 textOutput("tdap_dataRedFlags"),
#                 hr(),
#                 
#                 # assessment
#                 h4("Assessment:"),
#                 textOutput("tdap_assessment"),
#                 hr(),
#                 
#                 # plan
#                 h4("Plan:"),
#                 textOutput("tdap_plan"),
#                 hr(),
#                 
#                 # Export DAP note
#                 downloadButton(
...
#                 downloadButton("tdap_dr", label = "Generate letter", class = 'btn-success')
#             )
#         )
#     )
)

tdap_server <- function(input, output) {
#     # allergies selection
...
#     )
#     
#     # prior treatment to tdap medication selection
#     updateSelectizeInput(
#         inputId = "tdap_priorMed",
#         choices = drugs,
#         server = TRUE,
...
#     # Data
#     ## Hx
#     output$tdap_dataHx <- renderText({
#         # age
#         age <- trunc(as.Date(input$ptDOB) %--% Sys.Date() / years(1))
#         
#         # Preg & breastfeeding
#         if (input$tdap_pbf == "No") {
#             pbf <<- "Patient is not pregnant or breastfeeding. "
#         } else {
#             if (input$tdap_pbf == "Pregnant") {
#                 pbf <<- paste0("Patient is ", input$tdap_pregWks, " weeks pregnant (REFER). ")
#             } else {
...
#     })
#     
#     output$tdap_dataHx2 <- renderText({
#         # Previous episodes
#         if (input$tdap_prevEpisodes == "No") {
...
#         if (input$tdap_recentEpisode == "No") {
...
#         if (input$tdap_abnormalStructure == "No") {
...
#     })
#     
#     output$tdap_dataHx3 <- renderText({
#         ## Medication hx
#         if (input$tdap_immunosuppressors == "No") {
...
#         }
#         
#         if (length(input$tdap_allergies) > 0) {
#             allergies <<- paste0("Patient is allergic to ", paste0(tolower(input$tdap_allergies), collapse = ", "), ". ")
#         } else {
#             allergies <<- "Patinet reported no known allergies. "
#         }
#         
#         if (input$tdap_prior == "No") {
#             prior <<- "Patietn did not use drug therapy for UTI before. "
#         }
#         if (input$tdap_prior == "other") {
#             prior <<- paste0(
#                 "Upon asking about the prior treatment of UTI, patient indicated that",
#                 input$tdap_priorOther, ". "
#             )
#             
#         } else {
#             prior <<- paste0("Patient used ", tolower(input$tdap_priorMed), 
#                             " for UTI before where it was ", input$tdap_effect,
#                             " and ", input$tdap_tolerance, ". "
#             )
#         }
#         
...
#     
#     ## Clinical presentation
#     output$tdap_dataSx <- renderText({
#         
#         if (length(input$tdap_sxs) > 0) {
#             sxs <<- paste0(tolower(input$tdap_sxs), collapse = ", ")
#         } else {
#             sxs <<- "no symptoms"
#         }
#         
...
#                 paste0(tolower(input$tdap_pyelo), collapse = ", "), " (REFER). "
#             )
#         }
#         
#         if (length(input$tdap_othersxs) == 0){
...
#                 paste0(tolower(input$tdap_othersxs), collapse = ", "), " (REFER). "
#             )
#         }
#         
#         # render text
...
#     
#     # DAP Assessment
#     output$tdap_assessment <- renderText({
#         if (input$tdap_rationale1 == "" & input$tdap_decision == "Treat") {
...
#         } else {
#             if (input$tdap_rationale2 == "" & input$uti_decision == "Refer") {
...
#             } else {
#                 if (input$tdap_decision == "Treat") {
#                     assessment <<- paste0(input$tdap_decision, ": ", input$uti_rationale1)
#                 } else {
#                     assessment <<- paste0(input$tdap_decision, ": ", input$uti_rationale2)
#                 }
#             }
#         }
#         
...
#     
#     # DAP plan
#     output$tdap_plan <- renderText({
#         ## Treatment
#         ttt <<- paste0(
#             "After discussing the possible therapetdapc alternatives for UTI treatment with the patient, I prescribed ", 
...
#         ## Follow up
#         fu <<- paste0(
#             "I will follow up with patient in ", input$tdap_fuSchedule, " ",
#             input$tdap_fuUOT, " ", tolower(input$uti_fuMeans), " on the folowing: ",
#             paste0(input$tdap_fuItems, collapse = ", "), ". "
#         )
#         
#         # render text
#         plan <<- paste0(ttt, fu)
...
#     output$tdap_exportDAP <- downloadHandler(
#         filename = "tdap_dap.html",
#         content = function(file) {
#             tempReport <- file.path(tempdir(), "tdap_dap.Rmd")
#             file.copy("./tdap/uti_dap.Rmd", tempReport, overwrite = TRUE)
#             
...
#     output$tdap_dr <- downloadHandler(
#         filename = "tdap_dr.html",
#         content = function(file) {
#             tempReport <- file.path(tempdir(), "tdap_dr.Rmd")
#             file.copy("./tdap/uti_dr.Rmd", tempReport, overwrite = TRUE)
#             
#             # passing input parameters
#             params <- list(
#                 prescriber_name = input$pharmacist,
